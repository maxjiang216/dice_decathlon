...
pub fn solve_first_mover(
    attempt: &dyn AttemptEngine,
    axis: Axis,
    after: &(dyn Fn(i32) -> f64 + Sync),
) -> Vec<f64> {
    let bests = attempt.max_score() + 1;
    axis.iter()
        .collect::<Vec<_>>()
        .into_par_iter()
        .map(|d| {
...
pub fn measure(
    ...
    super::compress::Stats {
        ...
        // Enough to name any action any engine offers.
        action_bits: 5,
...
pub fn solo_expected_value(attempt: &dyn AttemptEngine) -> f64 {
    let bests = attempt.max_score() + 1;
